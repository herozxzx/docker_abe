...
auth2 CreateAuthority2(int i)
{
    ...
    static secrets tmp_sk[MAX_SIZE];
...
void Encrypt(cipher2 &ciphertext, string raw_message, vector<int> &w, vector<element_t *> &pks)
{
    int non_opt_w = non_opt_h.size();
    int opt_w = opt_h.size();
    chrono::system_clock::time_point start, end;
    ...
    static element_t tmp_rTs[MAX_SIZE];
...
void Decrypt(cipher2 &ciphertext, char *dec_m, vector<int> &cond, vector<int> &cond_c, map<int, secrets *> &sks)
{
    int non_opt_w = non_opt_h.size();
    int opt_w = opt_h.size();
    chrono::system_clock::time_point start, end;
    ...
    element_set1(skl.sk1);
    element_set1(skl.sk2);
    element_set1(skl.sk3);
    int count = 0;
    for (int &attr_c : cond)
    {
...
void P_Decrypt(cipher2 &ciphertext, char *dec_m, vector<int> &cond, map<int, secrets *> &sks)
{
    ...
    cout << " " << threads.size() << " + 1 threads started." << endl;

    // main-thead is caluculating 3 pairings
    element_t pair1, pair2, pair3, m;
    element_init_GT(pair1, pairing);
    element_init_GT(pair2, pairing);
    element_init_GT(pair3, pairing);
...
int main(int argc, char **argv)
{
    ...
    vector<int> cc{0, 0, 0};
    Decrypt(ct, dec_message1, user1, cc, sks);
